...
frequency(args...) = begin
    ...
    define(n => count())
    partition(name = all)
    define(`%` => floor(100 * n / all.sum(n)))
    order(n.desc())
end

array_frequency(expr; name = $(FunSQL.label(convert(FunSQL.SQLQuery, expr)))) = begin
    partition(name = all)
    cross_join(from(explode_outer($expr), columns = [$name]))
    group($name)
    define(n => count())
    define(`%` => floor(100 * n / any_value(all.count())))
    order(n.desc())
end

histogram(expr, bins = 20; name = $(FunSQL.label(convert(FunSQL.SQLQuery, expr)))) = begin
    define(value => $expr)
    group()
    as(source)
    over(
...
percentiles(expr, qs = [0, 0.025, 0.25, 2.5, 25, 50, 75, 97.5, 99.75, 99.975, 100]; name = $(FunSQL.label(convert(FunSQL.SQLQuery, expr)))) = begin
    group()
...
validate_foreign_key(source::FunSQL.SQLQuery, source_columns::Vector{Symbol}, target::FunSQL.SQLQuery, target_columns::Vector{Symbol}) = begin
	...
	foreign_key => $(_foreign_key_name(source, source_columns, target, target_columns)),
        is_valid => coalesce(!bool_or(source_key_is_not_null && !target_is_present), true),
        n => sum(source.count()),
    ...
    define(pct_tgt_linked => floor(100 * n_tgt_linked / n_tgt), after = n_tgt_linked)
end
